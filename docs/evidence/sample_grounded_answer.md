# Sample Grounded Answer (from Eval Corpus)

**Question:** *What is the maximum permanent link length in horizontal cabling according to TIA-568?*

**System Answer (answered):**

> The horizontal permanent link is limited to **90 metres**.

## Citations

> ```json
> {
>   "chunk_id": "c-12",
>   "source": "01_horizontal_cabling.pdf",
>   "page": 4,
>   "snippet": "The horizontal permanent link is limited to 90 m"
> }
> ```

**Verdict:** Citation verified. The exact snippet appears verbatim in chunk `c-12` of document `01_horizontal_cabling.pdf`, page 4. The answer is grounded.

---

Synthetic, redacted. Canonical example from the S04 eval fixtures showing correct citation behaviour. Real production transcripts replace this placeholder after the closeout commit.